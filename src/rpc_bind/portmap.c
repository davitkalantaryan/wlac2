...
struct pmaplist* s_pmaplist;

void sun_rpc_report(const char*);  /// Added by DK
static BOOL reg_service(struct svc_req *rqstp, SVCXPRT*xprt);
...
PortMap(void)
{
	...
	if ((sock = (int)socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) == INVALID_SOCKET) {
		sun_rpc_report("cannot create socket\n");
		return 1;
		exit(1);
	}
	...
	if (bind(sock, (struct sockaddr *)&addr, len) != 0) {
		sun_rpc_report("cannot bind\n");
		return 1;
		exit(1);
	}

	if ((xprt = svcudp_create(sock)) == (SVCXPRT *)NULL) {
		sun_rpc_report("couldn't do udp_create\n");
		return 1;
		exit(1);
	}
	...
	if ((xprt = svctcp_create(sock, 0, 0)) == (SVCXPRT *)NULL) {
		sun_rpc_report("couldn't do tcp_create\n");
		return 1;
		exit(1);
	}
	...
	svc_run();
	sun_rpc_report("run_svc returned unexpectedly\n");
	UpdateStatus(-1,-1);
	return 1;
}
	...
	u_long vers;
{
	...
	for (pml = s_pmaplist; pml != NULL; pml = pml->pml_next) {
	...
	SVCXPRT *xprt;
{
	...
	case PMAPPROC_UNSET:
		...
		if (!svc_getargs(xprt, xdr_pmap, &reg))
			...
			ans = 0;
			for (prevpml = NULL, pml = s_pmaplist; pml != NULL; ) {
