...
returnPoint:
	if(pcBuffer){
		...
		if(  ((a_argType== argumentParser::argType::rightArg)||(a_argType==argumentParser::argType::bougthArg)) && a_defRight)
		{aPair.infoStr = std::string("(default:")+ a_defRight+std::string(") ");}
		else {aPair.infoStr="";}
...
void common::argument_parser::ParseCommandLine(int& a_argc, char** a_argv)
{
	...
	for (i=0; i<a_argc;){
        ...
        for(j=1;j<unStrLen;++j){
			cTmp = (a_argv[i])[j];

            (a_argv[i])[j]=0;
            if(!m_htOptionsInAndOut.count(a_argv[i])){(a_argv[i])[j]=cTmp;continue;}
            pInput = m_htOptionsInAndOut[a_argv[i]];
            (a_argv[i])[j]=cTmp;
			if (!pInput || pInput->isFound) {continue; }

            SetRightOptionValueIfNeeded(pInput,a_argv[i]+j);
            bFound = true;
            break;
		}
        if(bFound && (i < (--a_argc))){
